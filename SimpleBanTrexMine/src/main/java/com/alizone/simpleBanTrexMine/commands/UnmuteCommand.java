...
public class UnmuteCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (SimpleBanTrexMine.instance.mutes.contains(uuid)) {
            SimpleBanTrexMine.instance.mutes.set(uuid, null);
            SimpleBanTrexMine.instance.saveMutes();
            String msg = SimpleBanTrexMine.instance.getConfig().getString("messages.unmute")
                    .replace("{player}", target.getName());
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', msg));
        } else {
            sender.sendMessage(ChatColor.YELLOW + "Player is not muted.");
        }
